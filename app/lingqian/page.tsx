...
function getLevelColor(level: string): string {
  if (level.includes("大吉") || level.includes("上上")) return "bg-red-500/15 text-red-500 border-red-500/30";
  if (level.includes("上") || level.includes("吉")) return "bg-gold/15 text-gold border-gold/30";
  if (level.includes("中")) return "bg-blue-500/15 text-blue-600 border-blue-500/30";
  if (level.includes("末")) return "bg-gray-500/15 text-gray-500 border-gray-500/30";
    ...
    <div className="page-container">
      ...
      <div className="page-grid">
        ...
        <div className="space-y-4">
          ...
          <div className="card-classic overflow-hidden p-6">
            <div className="flex flex-col items-center">
              <div className={`mb-4 transition-transform ${isShaking ? "animate-shake" : ""}`}>
                <div className="relative flex h-40 w-20 flex-col items-center">
                  <div className="absolute bottom-0 left-1/2 h-32 w-16 -translate-x-1/2 rounded-b-2xl bg-gradient-to-b from-amber-700 to-amber-900 border border-amber-600/50" />
                  <div className="absolute left-1/2 top-0 h-4 w-20 -translate-x-1/2 rounded-t-lg bg-amber-700 border border-amber-500/50" />
                  {!result && (
                    <>
                      <div className="absolute left-[38%] top-2 h-24 w-1.5 -translate-x-1/2 rounded-t bg-amber-200/80" />
                      <div className="absolute left-[48%] top-1 h-28 w-1.5 -translate-x-1/2 rounded-t bg-amber-100/80" />
                      <div className="absolute left-[58%] top-3 h-22 w-1.5 -translate-x-1/2 rounded-t bg-amber-200/60" />
                    </>
                  )}
                </div>
              </div>
              ...
              <div className="mb-4">
                <span className="rounded-full bg-green-500/15 px-3 py-1 text-sm text-green-600">
                  🎁 免费不限次数
                </span>
              </div>
          ...
          {result && !isShaking && (
            <>
              ...
              {isRevealed && <DonateSection variant="compact" text="解签已毕，随喜打赏，福报回向 🙏" />}
            </>
          )}
        ...
        <aside className="page-sidebar">
          <div className="card-classic p-5">
            ...
            {history.length === 0 ? (
              <p className="py-8 text-center text-xs text-text-muted">尚无记录，诚心抽一签吧</p>
            ) : (
              <div className="space-y-2">
                {history.map((record, idx) => (
                  <div key={idx} className="flex items-center justify-between rounded-lg border border-border bg-bg-input p-3">
                    ...
                    <button onClick={() => deleteRecord(idx)} className="ml-2 flex h-7 w-7 flex-shrink-0 items-center justify-center rounded-lg text-text-muted hover:bg-red-500/10 hover:text-red-500">
